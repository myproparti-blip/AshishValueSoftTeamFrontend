...
const DashboardPage = ({ user, onLogout, onLogin }) => {
    ...
    useEffect(() => {
        // Duration update interval - throttle to every 5 seconds for better performance
        const durationInterval = setInterval(() => {
            calculateTimeDurations(files);
        }, 5000); // Update durations every 5 seconds instead of 1 second
    ...
    const fetchFiles = async (isInitial = false, showLoadingIndicator = true) => {
        try {
            ...
            const [bofResponse, ubiApfResponse] = await Promise.all([
